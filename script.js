...
function renderOrders() {
    ...
    activeOrders.forEach(order => {
        ...
        if (order.endDate) {
            ...
            diffUnits = Math.ceil(diffTime / (1000 * 60 * 60 * window.getBaseHours()));
...
function renderEarnings() {
    ...
    completedOrders.forEach(order => {
        ...
        const diffUnits = Math.ceil(Math.abs(e - s) / (1000 * 60 * 60 * window.getBaseHours()));
...
function renderCompletedOrders() {
    ...
    completedOrders.forEach(order => {
        ...
        const diffUnits = Math.ceil(Math.abs(end - start) / (1000 * 60 * 60 * window.getBaseHours()));
        ...
        card.innerHTML = `
            ...
            <div class="order-detail">⏳ ${diffUnits} Unit(s) (${window.getBaseHours()}hr)</div>
            <div class="order-detail">⏱ ${durationStr}</div>
            ${order.extra ? `<div class="order-detail">📝 ${order.extra}</div>` : ''}
            ${proofHtml}
            <div class="action-row">
                ...
                <button class="btn-sm btn-delete-solid" onclick="deleteOrder('${order.id}')">Delete</button>
                <button class="btn-sm btn-invoice" onclick="previewInvoice('${order.id}')">Invoice</button>
            </div>
        `;
...
// Global Base Hours (default 12 if not set)
window.getBaseHours = () => {
    return parseInt(localStorage.getItem('zillow_base_hours') || '12', 10);
};

window.calculateTotalCost = (start, end, rate) => {
    const diffTime = Math.abs(end - start);
    const baseHours = window.getBaseHours();
    const diffUnits = Math.ceil(diffTime / (1000 * 60 * 60 * baseHours));
    return diffUnits * rate;
};

// Update Handler
window.applyBaseHours = () => {
    const input = document.getElementById('base-rate-hours');
    if (input) {
        window.updateBaseHours(input.value);
    }
};

window.updateBaseHours = (val) => {
    if (val && val > 0) {
        localStorage.setItem('zillow_base_hours', val);
        ...
        // Ideally re-render items if live update needed, but next action will catch it
        alert('Base rate scale updated. All Active, Booked, Expired and Completed orders have been recalculated.');
    } else {
        alert("Please enter a valid number of hours.");
    }
};

window.updateRatePlaceholders = () => {
    const hours = window.getBaseHours();
    const input = document.getElementById('new-product-rate');
    if (input) input.placeholder = `Rate/${hours}hr`;
};

// Init on load
document.addEventListener('DOMContentLoaded', () => {
    const input = document.getElementById('base-rate-hours');
    if (input) input.value = window.getBaseHours();
    updateRatePlaceholders();
});

// --- Shared Generator (Live Preview + Final) ---
// --- Shared Generator (Live Preview + Final) ---
...
window.generateInvoicePDF = async (order, prefs, isPreview = false) => {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();

    // Use Code (e.g. INR) instead of Symbol to avoid font issues (showing as ¹)
    const currencyCode = localStorage.getItem('zillow_currency') || 'USD';

    // -- Watermark --
    try {
        // Load specific logo for watermark
        ...
        const watermarkData = await getBase64FromUrl('zillow_logo_new.png');
        ...
        if (watermarkData) {
            ...
            // Draw Logo (Centered)
            const wmWidth = 100; // 100mm wide
            const logoProps = await getImageProperties(watermarkData);
            if (logoProps) {
                const wmHeight = wmWidth / logoProps.ratio;
                const wmX = (pageWidth - wmWidth) / 2;
                const wmY = (pageHeight - wmHeight) / 2;

                doc.addImage(watermarkData, 'PNG', wmX, wmY, wmWidth, wmHeight);

                // Draw Text below Logo
                doc.setFontSize(30);
                doc.setTextColor(150, 150, 150); // Gray
                doc.setFont("helvetica", "bold");
                doc.text("Zillow Rental Manager", pageWidth / 2, wmY + wmHeight + 15, { align: "center" });
            }

            // Restore state (opacity 1.0)
    ...
    if (prefs.logoData) {
        try {
            const props = await getImageProperties(prefs.logoData);
            if (props) {
                // Max box: 40x25
                const maxW = 40;
                const maxH = 25;
                let finalW = maxW;
                let finalH = maxW / props.ratio;

                if (finalH > maxH) {
                    finalH = maxH;
                    finalW = maxH * props.ratio;
                }

                doc.addImage(prefs.logoData, 'JPEG', leftX, topY, finalW, finalH);
                topY += (finalH + 5);
            } else {
                topY += 25; // fallback
            }
    ...
    doc.setFont("helvetica", "bold");
    const companyName = prefs.companyName || "Rental Service";
    doc.text(companyName, leftX, topY);
    topY += 6;
    ...
    let detailsText = "";
    if (prefs.companyDetails) detailsText += prefs.companyDetails + "\n";
    if (prefs.phone) detailsText += `Phone: ${prefs.phone}\n`;
    if (prefs.website) detailsText += `Web: ${prefs.website}`;

    const splitDetails = doc.splitTextToSize(detailsText, 80);
    doc.text(splitDetails, leftX, topY);

    // Right Column (Invoice Info + QR)
    let rightX = 195;
    let rightY = 20;
    ...
    // Check if it's the sample order from settings
    const isSample = order.id === 'SAMPLE-001';
    const invoiceNum = isSample ? "INV-SAMPLE" : `INV-${order.id.slice(-6).toUpperCase()}`;
    const invoiceDate = new Date().toLocaleDateString();

    doc.text(`Invoice #: ${invoiceNum}`, rightX, rightY, { align: "right" });
    doc.text(`Date: ${invoiceDate}`, rightX, rightY + 5, { align: "right" });

    // QR Code Placement (Below Date) - Fix Stretching
    if (prefs.qrData) {
        try {
            const props = await getImageProperties(prefs.qrData);
            if (props) {
                // Max box 30x30
                const maxS = 30;
                let qrW = maxS;
                let qrH = maxS / props.ratio;

                if (qrH > maxS) {
                    qrH = maxS;
                    qrW = maxS * props.ratio;
                }
                // Align right
                const qrX = rightX - qrW;
                doc.addImage(prefs.qrData, 'JPEG', qrX, rightY + 12, qrW, qrH);
            }
        } catch (e) { }
    }

    // -- Bill To --
    let yPos = Math.max(topY + (splitDetails.length * 4) + 15, 60);
    ...
    doc.text(order.userName, margin, yPos);

    yPos += 5;
    doc.setFontSize(10);
    doc.setTextColor(...textColor);
    doc.setFont("helvetica", "normal");
    // Construct Bill To Details
    const billToDetails = [];
    if (order.location) billToDetails.push(order.location);

    const phones = [];
    if (order.phone1) phones.push(order.phone1);
    if (order.phone2) phones.push(order.phone2);
    if (order.phone3) phones.push(order.phone3);

    if (phones.length > 0) billToDetails.push(`Phone: ${phones.join(', ')}`);

    doc.text(billToDetails.join('\n'), margin, yPos);

    // -- Table --
    yPos += 15;

    const start = new Date(order.startTime || order.startDate);
    const end = new Date(order.endTime || order.endDate);

    let durationStr = "0h";
    let totalCost = 0;

    if (!isNaN(start) && !isNaN(end)) {
        durationStr = window.calculateDuration(start, end);
        if (order.manualCost) {
            totalCost = parseFloat(order.manualCost);
        } else {
            totalCost = window.calculateTotalCost(start, end, order.rate);
        }
    }

    const dateFmt = { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' };

    const scale = window.getBaseHours();
    const tableData = [
        [
            `${order.productName} Rental\n${!isNaN(start) ? start.toLocaleDateString('en-US', dateFmt) : ''} - ${!isNaN(end) ? end.toLocaleDateString('en-US', dateFmt) : ''}`,
            durationStr,
            `${scale}hr`,
            `${currencyCode} ${order.rate}`,
            `${currencyCode} ${totalCost}`
        ]
    ];

    doc.autoTable({
        startY: yPos,
        head: [['Description', 'Duration', 'Hour Scale', 'Rate', 'Amount']],
        body: tableData,
        theme: 'plain',
        headStyles: {
    ...
    // -- Totals --
    // Position below the "Amount" column (column 3)
    ...
    // Approx X position for the last column content
    // Page width 210, margin 15, right align means text ends at margin
    doc.text(`Total: ${currencyCode} ${totalCost}`, 195, finalY, { align: "right" });

    // -- Footer Text Only (QR moved to top) --

    const footerText = prefs.footerText || "Thank you for your business!";
    doc.setFontSize(9);
    doc.setTextColor(...grayColor);
    doc.setFont("helvetica", "normal");
...
window.generateInvoice = async (id) => {
    ...
    // Look up product rate
    const product = products.find(p => p.name === order.productName);
    if (product) {
        order.rate = product.rate;
    }

    const prefs = JSON.parse(localStorage.getItem('zillow_invoice_prefs')) || {};
    await window.generateInvoicePDF(order, prefs, false);
}
...
window.previewInvoice = async (id) => {
    ...
    const prefs = JSON.parse(localStorage.getItem('zillow_invoice_prefs')) || {};
    await window.generateInvoicePDF(order, prefs, true); // true for preview
}
...
window.generatePDFReport = () => {
    ...
    const monthlyTableData = sortedMonths.map(month => [month, `${getCurrencySymbol()}${monthlyTotals[month]}`]);
    ...
    sortedMonths.forEach(month => {
        ...
        Object.entries(productsInMonth).forEach(([prodName, amount]) => {
            productTableData.push([month, prodName, `${getCurrencySymbol()}${amount}`]);
        });
    });
...
function renderAppSettings() {
    ...
    if (currencySelect) {
        ...
        currencySelect.addEventListener('change', (e) => {
            ...
            updateStaticCurrencyLabels();
            renderOrders();
            renderSettings(); // Update Product Rates List
            renderProducts(); // Update New Rental Dropdown
            renderEarnings();
            if (typeof renderCompletedOrders === 'function') renderCompletedOrders();
            renderExpenses();
    ...
    // 3. Invoice Preferences Logic
    const invPrefsCard = document.getElementById('invoice-prefs-card');
    const invForm = document.getElementById('invoice-settings-form');

    if (invPrefsCard) {
        invPrefsCard.onclick = () => {
            navTo('invoice-settings');
            // Load existing prefs
            const prefs = JSON.parse(localStorage.getItem('zillow_invoice_prefs')) || {};
            document.getElementById('inv-company-name').value = prefs.companyName || '';
            document.getElementById('inv-company-details').value = prefs.companyDetails || '';
            document.getElementById('inv-footer-text').value = prefs.footerText || '';
            document.getElementById('inv-phone').value = prefs.phone || '';
            document.getElementById('inv-website').value = prefs.website || '';

            // Show existing images status (Optional enhancement: Show preview if Base64 exists)
            const logoPreview = document.getElementById('inv-logo-preview');
            if (prefs.logoData) {
                logoPreview.innerHTML = `<img src="${prefs.logoData}" style="max-height:50px;"> <br> <small>Current Logo</small>`;
                logoPreview.style.display = 'block';
            } else {
                logoPreview.style.display = 'none';
            }

            const qrPreview = document.getElementById('inv-qr-preview');
            if (prefs.qrData) {
                qrPreview.innerHTML = `<img src="${prefs.qrData}" style="max-height:50px;"> <br> <small>Current QR</small>`;
    ...
    if (previewBtn) {
        previewBtn.onclick = async (e) => {
            ...
            // 1. Gather current form data (unsaved)
            const prefs = {
                companyName: document.getElementById('inv-company-name').value,
                companyDetails: document.getElementById('inv-company-details').value,
            ...
            // Handle Files (Need to read if new, or load old if existing)
            const logoInput = document.getElementById('inv-logo-input');
            if (logoInput.files.length > 0) {
                prefs.logoData = await getBase64(logoInput.files[0]);
            } else {
                const saved = JSON.parse(localStorage.getItem('zillow_invoice_prefs')) || {};
                prefs.logoData = saved.logoData;
            }

            const qrInput = document.getElementById('inv-qr-input');
            if (qrInput.files.length > 0) {
                prefs.qrData = await getBase64(qrInput.files[0]);
            } else {
                const saved = JSON.parse(localStorage.getItem('zillow_invoice_prefs')) || {};
                prefs.qrData = saved.qrData;
            }
            ...
            if (window.generateInvoicePDF) {
                await window.generateInvoicePDF(dummyOrder, prefs, true);
            } else {
                // Temporary fallback if helper not ready
                console.log("Generating preview with local logic...");
                // Note: We expect generateInvoicePDF from the other replacement
            }
        };
    }
    ...
    if (invForm) {
        invForm.onsubmit = async (e) => {
            ...
            // Text Fields
            prefs.companyName = document.getElementById('inv-company-name').value;
            prefs.companyDetails = document.getElementById('inv-company-details').value;
            prefs.footerText = document.getElementById('inv-footer-text').value;
            prefs.phone = document.getElementById('inv-phone').value;
            prefs.website = document.getElementById('inv-website').value;
            ...
            localStorage.setItem('zillow_invoice_prefs', JSON.stringify(prefs));
